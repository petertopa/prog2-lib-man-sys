...
public class TestHarness {
	...
	public static void initialiseEngine() {
		...
		// return functions (check the "Assignments" page on Blackboard for
		// further explanation)
	...
	public static boolean test2() throws Exception {
		...
		String[] expected = {
				...
				"2000001:Java 1:4:7:VIDEO", "2000003:UML 1:6:7:VIDEO" };
		...
		try {
			...
			if (failureCheck(model.countBooks() == numOfBooks, true))
				return false;
			System.out.print("Checking the videos counting function ...");
			if (failureCheck(model.countVideos() == numOfVideos, true))
				return false;

			// check Library Collection string representation (with holding
			// present)
			LibraryCollection collection = model.getCollection();
			System.out.print("Library Collection string is: "
					+ collection.toString());
			if (failureCheck(
					collection.toString().equalsIgnoreCase(collectionString),
					true))
				return false;
	...
	public static boolean test3() throws Exception {
		...
		try {
			...
			// NOTE: model.getHolding() MUST return NULL when the holding cannot
			// be found
			System.out
			...
			// NOTE: model.getAllHoldings() MUST return NULL if there are no
			// holdings in the collection
			System.out
	...
	public static boolean test4() throws Exception {
		...
		try {
			...
			if (failureCheck(
					checkForHoldings(model.getBorrowedHoldings(),
			...
			if (failureCheck(
					model.calculateRemainingCredit() == standard_credit_expected,
					true))
				return false;
			...
			if (failureCheck(
					model.calculateRemainingCredit() == premium_credit_expected,
					true))
				return false;
	...
	public static boolean test6() throws Exception {
		...
		try {
			...
			if (failureCheck(
					checkForRecords(model.getBorrowingHistory(), expected),
					true))
				return false;
	...
	public static void test11() {
		...
		// remember here that System.out and System.err are now mapped to the
		// file.
